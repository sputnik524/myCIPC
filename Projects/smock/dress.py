...
if __name__ == "__main__":
    ...
    sim.add_shell_with_scale_3D_smock("input/"+smock_name +"/S4.obj","input/"+smock_name +"/S1.obj", Vector3d(0.5, 0.70, -0.25), Vector3d(0.065, 0.065, 0.065),\
        Vector3d(0, 0, 0), Vector3d(1, 0, 0), -90) 

    # 2. add the garment with identical size that to be sewed 
    sim.add_shell_with_scale_3D("input/"+smock_name +"/S3.obj", Vector3d(0.1, -0.2, 0.55), #Vector3d(0, 0.2, -0.85)
    ...
    meshCounter = sim.add_shell_with_scale_3D("input/woman_rot.obj", Vector3d(0.75, 0.5, 0.12), \
        Vector3d(2.7, 2.7, 2.7),Vector3d(0, 0, 0), Vector3d(1, 0, 0), 135) 
    ...
    sim.smock = True
    sim.smock_cons = 0.1
    sim.uniform_stitching_ratio = 0.3
