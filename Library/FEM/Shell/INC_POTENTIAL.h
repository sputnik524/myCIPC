...
{
    TIMER_FLAG("Compute_IncPotential_with_smock");
    value = 0;
    ...
    if constexpr (flow) {
        ...
        if (fiberStiffMult[0] > 0 || fiberStiffMult[1] > 0) {
            ...
            if (smock)
                Compute_Smock_Membrane_Energy(Elem, Elem_smock, staticSolve ? 1.0 : h, DBCb, X, nodeAttr, elemAttr, elemAttr_smock, elasticityAttr, elasticityAttr_smock, value);
            else
                std::cout << "Must compute smock constaints!" << std::endl;
        }
        if (bendingStiffMult) {
            Compute_Bending_Energy<T, dim, KL>(Elem, staticSolve ? 1.0 : h, edge2tri, edgeStencil, edgeInfo, thickness, bendingStiffMult, DBCb, X, nodeAttr, elemAttr, elasticityAttr, value);
        }
...
void Compute_IncPotential_Gradient(
    ...
    FIXED_COROTATED<T, dim - 1>& elasticityAttr_smock, bool smock)
{
    TIMER_FLAG("Compute_IncPotential_Gradient_with_smock");
    nodeAttr.template Fill<FIELDS<MESH_NODE_ATTR<T, dim>>::g>(VECTOR<T, dim>(0));
    ...
    if constexpr (flow) {
        ...
        if (fiberStiffMult[0] > 0 || fiberStiffMult[1] > 0) {
            ...
            if(smock)
                Compute_Smock_Membrane_Gradient(Elem, Elem_smock, staticSolve ? 1.0 : h, DBCb, X, nodeAttr, elemAttr, elemAttr_smock, elasticityAttr, elasticityAttr_smock);
            else
                std::cout << "Must compute smock constaints!" << std::endl;
        }
        if (kappa_s[0] > 0) {
    ...
    FIXED_COROTATED<T, dim - 1>& elasticityAttr_smock, bool smock)
{
    ...
    if constexpr (flow) {
        ...
        if (fiberStiffMult[0] > 0 || fiberStiffMult[1] > 0) {
            ...
            if(smock)
                Compute_Smock_Membrane_Hessian(Elem, Elem_smock, staticSolve ? 1.0 : h, projectSPD, DBCb, X, nodeAttr, elemAttr, elemAttr_smock, elasticityAttr, elasticityAttr_smock, triplets);
            else
                std::cout << "Must compute smock constaints!" << std::endl;
        }
        if (kappa_s[0] > 0) {
